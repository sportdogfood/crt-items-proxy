...
// Route: commit JSON files under /docs
// --- /docs/commit (mirror of /items/commit but forces docs/)
app.post("/docs/commit", express.json({ limit: "2mb" }), async (req, res) => {
  try {
    const { path, json, message, overwrite } = req.body || {};
    if (!path || typeof path !== "string") {
      return res.status(400).json({ error: "Missing 'path' string" });
    }
    if (!json || typeof json !== "object" || Array.isArray(json)) {
      return res.status(400).json({ error: "Missing 'json' object" });
    }

    // normalize + enforce docs/
    const rel = String(path).trim().replace(/^\/+/, "");        // drop leading /
    const clean = rel.replace(/^docs\//, "");                    // strip existing docs/ if present
    const repoPath = `docs/${clean}`;                            // force docs/

    const result = await commitToRepo({
      upstreamBase: UPSTREAM_BASE,
      repoPath,                          // e.g. docs/blogs/.../file.json
      contentObj: json,                  // { ...full JSON... }
      message: message || `chore: commit ${repoPath} via proxy`,
...
app.get("/docs/*", async (req, res) => {
  try {
    ...
    const rel = String(req.params[0] || "").trim().replace(/^\/+/, "");
    const clean = rel.replace(/^docs\//, "");
    const repoPath = `docs/${clean}`;

    // build upstream URL (UPSTREAM_BASE ends at repo root)
    // e.g. https://raw.githubusercontent.com/.../main/
